  ...
  useEffect(() => {
    const fetchRoster = async () => {
      try {
        setLoading(true);
        setError(null);
        const data = await getTeacherClassroomStudents();
        setClassrooms(data);
      } catch (err: any) {
        setError(
          err?.response?.data?.message ||
            "Failed to load students roster. Please try again."
        );
      } finally {
        setLoading(false);
      }
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="text-slate-700 font-medium">Loading student rosters...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 pb-12">
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-slate-600 text-sm mt-0.5">
              View student membership across all classrooms you created
            </p>
          </div>
        </div>
      ...
      <motion.div
        ...
        className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4"
      >
        {/* Search Input */}
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <select
            ...
            className="w-full md:w-64 py-2.5 px-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all cursor-pointer"
          >
                              ...
                              >
                                ...
                                <td className="py-3.5 font-medium text-slate-900">
                                  <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 text-white font-bold text-xs flex items-center justify-center shadow-xs">
                                      {initial}
                                    </div>
